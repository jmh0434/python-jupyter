...
def seoul_shop(result):
    ...
    for page in range(1, 4):  # 1페이지부터 3페이지까지
        wd.get('https://thesool.com/front/publication/M000000059/list.do?page=%d&bbsId=A000000056&searchKey=&searchString=&searchCategory=' %page)
        time.sleep(3)
        try:
            ...
            for link in link_list[:-1]:
                ...
                time.sleep(1)
                result.append([shop_name]+[shop_address] +
                              [shop_time]+[shop_phone]+[shop_menu])  # 한 리스트에 담기
        except:
            continue
    return
